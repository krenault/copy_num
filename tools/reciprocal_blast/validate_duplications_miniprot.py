...
def parse_miniprot_gff(gff_path: Path, min_identity: float = 50.0) -> Dict:
    """Parse miniprot GFF to count ALL distinct genomic loci for duplication detection.

    For duplications, we want to count all high-scoring hits, not just merged ones.
    Reports both raw count and merged count (for tandem duplicates).
    """
    loci = []
    ...
    # Count raw loci (all high-scoring hits)
    n_raw_loci = len(loci)

    # Also merge nearby loci (within 50kb) to count distinct genomic regions
    merged = []
    ...
    return {
        'n_loci': len(merged),           # Merged count (distinct regions)
        'n_raw_loci': n_raw_loci,         # Raw count (all hits)
...
def main():
    ...
    parser.add_argument("--toga-matrix", required=True, help="TOGA duplication binary matrix")
    ...
    for gene in genes:
        ...
        for species, genome_path in genome_paths.items():
            ...
            # Get TOGA prediction (duplication: 1=duplicated, 0=single copy)
            # Handle species name suffixes (e.g., Bradypus_torquatus vs Bradypus_torquatus_1)
            toga_dup = toga_data.get(gene, {}).get(species, None)
            if toga_dup is None:
            ...
            # Record result - concordance for DUPLICATION validation:
            # miniprot loci > 1 (multiple copies) should match TOGA duplication == 1
            result = {
                'gene': gene,
                'species': species,
                ...
                'toga_duplication': toga_dup,
                'concordant': 'yes' if (mp_result['n_loci'] > 1) == (toga_dup == 1) else 'no'
            }
            results.append(result)

            status = "✓" if result['concordant'] == 'yes' else "✗"
            dup_status = "DUP" if mp_result['n_loci'] > 1 else "single"
            print(f"  {status} {species}: miniprot={mp_result['n_loci']} loci ({mp_result['n_raw_loci']} raw), TOGA_dup={toga_dup} [{dup_status}]")
